...
| A-05 | "Read as" chips | Shows every field the app understood. Blue means from your question, a solid outline means from a tap, dashed means a default. | ✅ | `app.js` `renderRead` | scenario tests |
...
| ID | Feature | What the user sees or does | Status | Lives in | Tested |
|---|---|---|---|---|---|
| V-01 | Verdict stamp | **Burned** (NASA saw it burn under matching conditions) or **No data** (no test matches), as a large coloured word with the test count under it. Burned and Mixed are orange, No flame held is grey and No data is amber. None of them is ever green. | ✅ | `renderVerdict` | scenario tests |
...
| V-03 | Evidence match list | In its own card: your cabin next to what the tests covered, for material, gravity, O₂ and pressure, plus airflow and thickness when you gave them. Each row is marked Match or Gap. | ✅ | `checksFor`, `#match` | scenario tests |
...
| V-06 | Chamber view | An animated flame beside the verdict: a blue sphere in microgravity, a dashed outline with "?" at Moon or Mars gravity, labelled **Illustration**, with gravity and air readouts and a caption under it | ✅ | `app.js` canvas | manual |
...
| E-04 | Key numbers | Four cards under the verdict: tests matched, burn-time range, and the fastest and slowest spread with their test IDs. For fabric and Nomex they show the test count and an outcome bar (burned, went out, blew out, didn't ignite) whose parts add up to every test. Each card opens its rows. Hidden when the answer is No data. | ✅ | `renderKpis` | scenario tests (values) |
