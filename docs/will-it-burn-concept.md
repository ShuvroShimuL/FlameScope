...
Run it with `node server.mjs`, then open <http://127.0.0.1:3000/burn/>. The technical walkthrough is in [will-it-burn-technical.md](will-it-burn-technical.md).
...
Both views read the same NASA table through `lib/evidence.mjs`, so no data work is duplicated. FlameScope at `/` stays the research view for comparing tests and exporting briefs. Will It Burn? at `/burn/` is the front door that answers one question fast.
